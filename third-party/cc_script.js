...
(function (window, $) {
    ...
    $.fn.ccCarousel = function (options) {
        ...
        var defaults = {
            slider: $('.cc_slider'), //dynamic div
            sliderArray: $('.cc_slider div'), //array of images in the slider
            wrapperWidth: $('.cc_galleryWrapper').width(), //initializing the wrapper gallery width
            dynamicWidth: $('.cc_dynamic').width() //initializing the dynamic wrapper width
        };
        ...
        function bindEvents() {
            /*jshint validthis: true */
            $(this).find('.cc_leftDirection').on('mousedown', moveToLeft);
            $(this).find('.cc_rightDirection').on('mousedown', moveToRight);
            $(this).find('.cc_leftDirection').bind('mouseover', leftMouseover);
            $(this).find('.cc_rightDirection').bind('mouseover', rightMouseover);
            $(this).find('.cc_leftDirection').bind('mouseup', leftMouseup);
            $(this).find('.cc_rightDirection').bind('mouseup', rightMouseup);
            $(this).find('.cc_leftDirection').bind('mouseout', mouseout);
            $(this).find('.cc_rightDirection').bind('mouseout', mouseout);
            $(this).find('.cc_slider img').on('click', imageClick);
        }
